...
import { validateImagePrompt, generateImageWorkersAI, generateImageWorkersAI_img2img } from '../_lib/imageGen.mjs';
...
export async function onRequestPost(context) {
  ...
  if (visualDecomposeCmd) {
    ...
    // Stage 2 now tries TWO paths, in order:
    //   (a) img2img against the REAL uploaded photo (firstImage.data passed
    //       straight through as the conditioning image) — this is the
    //       actual fix for "the output looks nothing like my photo": the
    //       model sees the real pixels, not just Gemini's text account of
    //       them. See imageGen.mjs's generateImageWorkersAI_img2img header
    //       comment for the model choice, the pricing caveat, and a
    //       correction note (a parameter-format bug from an earlier version
    //       of that function meant it was never actually seeing the photo
    //       either — fixed there, not repeated here).
    //   (b) the original blind text-to-image call (Gemini's description
    //       alone, no photo), tried ONLY if (a) fails for any reason —
    //       oversized/rejected input, the model's own documented black-
    //       image failure mode, a timeout, anything. This keeps the
    //       "always return something" guarantee from before this change
    //       rather than trading reliability for the photo-conditioning fix.
    let imageResult = await generateImageWorkersAI_img2img(env.AI, firstImage.data, finalPrompt);
    let visualDecomposeMethod = 'img2img';
    if (!imageResult.ok) {
      console.warn('[vision.js] visual-decompose img2img failed, falling back to text-only —',
        imageResult.errStatus, imageResult.errBody);
      imageResult = await generateImageWorkersAI(env.AI, finalPrompt);
      visualDecomposeMethod = 'text-only-fallback';
    }
    if (!imageResult.ok) {
      console.error('[vision.js] visual-decompose generate step failed on both paths:', imageResult.errStatus, imageResult.errBody);
