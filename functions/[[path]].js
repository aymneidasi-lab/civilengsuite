 ...
 *
 *
 * 2026-06-07 v14 — Mobile download protection: async-guard race-condition fix (M1c, M2b, XOR-check):
 *
 *   ROOT CAUSE — remaining gap in v13:
 *     Both M1a and M2 terminate on unauthorized origin via
 *     window.location.replace() inside the catch block when document.open()
 *     fails. window.location.replace() is ASYNCHRONOUS — JS execution in
 *     the current script context continues after the call. On the bootstrap
 *     side: the HTML parser resumes, reaches <body>, and the XOR decoder
 *     <script> fires — document.write()ing the real page before the
 *     navigation completes (or permanently when the device is offline or
 *     the WebView blocks external navigation).
 *
 *   [M1c] bootstrapOriginGuard: set window._ces_blocked=1 SYNCHRONOUSLY
 *         and IMMEDIATELY when unauthorized origin is detected — BEFORE the
 *         try/catch block. In the catch block: register a DOMContentLoaded
 *         listener that force-shows #_ces_cr_body (M1b) with display:flex
 *         so offline WebView users see copyright instead of a blank screen.
 *         window.location.replace() is kept inside a nested try/catch so a
 *         navigation failure does not suppress the visibility of M1b.
 *
 *   [XOR] XOR decoder in bootstrap shell: add
 *         if(window._ces_blocked){return;} as the FIRST statement inside
 *         the IIFE. M1a/M1c is a synchronous parser-blocking <head> script;
 *         _ces_blocked is guaranteed set before the <body> XOR decoder
 *         script is reached by the parser. If M1a's document.open()
 *         succeeded the document was already replaced — the XOR script never
 *         runs. If document.open() failed, _ces_blocked=1 stops the decoder
 *         unconditionally. Real HTML is never decoded on unauthorized opens.
 *
 *   [M2b] _m2Code (decoded-HTML guard): set window._ces_blocked=1
 *         SYNCHRONOUSLY. In the catch block: inject
 *         html,body{visibility:hidden!important} via
 *         document.createElement('style') appended to documentElement —
 *         safe during head parsing (documentElement exists at that point).
 *         Hides all rendered content even if window.location.replace()
 *         fails (offline/WebView). Navigation kept in nested try/catch.
 *
...
export async function onRequest(context) {
  ...
  const _m2Code =
    ...
    + `catch(e){`
    + `try{var _st=document.createElement('style');_st.textContent='html,body{visibility:hidden!important;overflow:hidden!important}';document.documentElement.appendChild(_st);}catch(_e2){}`
    + `try{window.location.replace(_ao+'${_sharedCrRP}/');}catch(_e3){}`
    + `}`
    + `}`
    + `})();`;
  ...
  const bootstrapOriginGuard =
    ...
    + `var _d=document.getElementById('_ces_cr_body');`
    + `if(_d)_d.style.cssText='display:flex!important;position:fixed;top:0;left:0;width:100%;height:100%;z-index:2147483647;background:#0A1A2E;align-items:center;justify-content:center;';`
    + `});`
    + `try{window.location.replace(_ao+'${_sharedCrRP}/');}catch(e2){}`
    + `}`
    + `}`
    + `})();`
    + `\u003c/script>`;
  ...
  const bootstrap = `<!DOCTYPE html><html><head>`
    ...
    + webMCPBootstrap
    + `<script nonce="${cspNonce}">`
    + `(function(){if(window._ces_blocked){return;}try{`
    + `var p="${payload}";`
    + `var b=atob(p);`
