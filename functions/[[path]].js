 ...
 * All other requests (images, CSS, sitemap…) pass through via context.next()
 * to Cloudflare's CDN static serving + _redirects rules.
 *
 * Uses ONLY Web APIs — no Node.js, no fs, no require().
 * api/decrypt.js is left completely untouched in the repo.
 *
...
// ── Route table (mirrors api/decrypt.js ROUTES exactly) ──────────────────────
const ROUTES = [
  {
    prefix: '/', exact: true,
...
// ── In-memory rate limiter (best-effort within Worker instance) ───────────────
...
// Chunked base64 — safe for large payloads (avoids stack overflow on spread)
...
// Format matches Python encryptor: base64(iv) + "." + base64(ciphertext + 16-byte GCM tag)
async function decryptEnc(encData, keyHex) {
  ...
  const ct  = b64ToU8(encData.slice(dot + 1));   // Web Crypto reads GCM tag from tail automatically
...
function buildProtectionBundle(pageFilename) {
  // Copyright HTML served on Ctrl+S (pre-encoded server-side)
  ...
  // ── Layer 5: DevTools detection (width-delta + debugger-timing, both methods) ──
  ...
  // ── Layer 1: Right-click block (capture phase — catches Shift+right-click in Firefox) ──
  + `document.addEventListener('contextmenu',function(e){e.preventDefault();e.stopPropagation();return false;},true);`

  // ── Layer 2: Keyboard shortcuts (capture phase — catches browser Edit menu too) ──
  ...
  // ── Layer 3: Copy/cut — replace clipboard with copyright notice ──────────────
  ...
  // ── CSS: selectstart prevention (INPUT/TEXTAREA/contentEditable exempted) ────
  + `document.addEventListener('selectstart',function(e){`
  + `if(e.target&&(e.target.tagName==='INPUT'||e.target.tagName==='TEXTAREA'||e.target.isContentEditable))return;`
  + `e.preventDefault();return false;},true);`

  // ── Layer 2 (Tier 1): Ctrl+S → copyright download ────────────────────────────
  + `document.addEventListener('keydown',function(e){`
  + `if((e.ctrlKey||e.metaKey)&&(e.keyCode===83||e.which===83)){`
  + `e.preventDefault();e.stopPropagation();`
  + `try{var _eb='${crB64}';`
  + `var _bn=atob(_eb);var _ba=new Uint8Array(_bn.length);`
  + `for(var i=0;i<_bn.length;i++)_ba[i]=_bn.charCodeAt(i);`
  + `var _dc=new TextDecoder('utf-8').decode(_ba);`
  + `var _bl=new Blob([_dc],{type:'text/html'});`
  + `var _ul=URL.createObjectURL(_bl);var _al=document.createElement('a');`
  + `_al.href=_ul;_al.download='${pageFilename}';_al.click();`
  ...
  // ── Layer 8: Watermark MutationObserver guard (attribute + childList + interval) ──
  + `try{var _wm=document.getElementById('ces-watermark');`
  + `if(_wm){var _ws=_wm.style.cssText;`
  + `var _mo=new MutationObserver(function(){`
  + `if(!document.getElementById('ces-watermark'))document.body.appendChild(_wm);`
  + `_wm.style.cssText=_ws;});`
  + `_mo.observe(document.body,{childList:true,subtree:false});`
  + `_mo.observe(_wm,{attributes:true,attributeFilter:['style','class','hidden']});`
...
export async function onRequest(context) {
  ...
  const route = (path === '' || path === '/' || path === '/index.html')
    ? ROUTES[0]
    : ROUTES.slice(1).find(r => path === r.prefix || path.startsWith(r.prefix + '/'));
  ...
  const ip = (request.headers.get('CF-Connecting-IP') ||
               request.headers.get('X-Real-IP') || 'anon').split(',')[0].trim();
  if (!allowRequest(ip))
    return errResponse(429, 'Too Many Requests', 'Rate limit exceeded. Please wait a moment and try again.');
